...
function preprocess() {
     ...
     echo python $preprocess_bin --path_base "${dicom_data}/${sub_dir}" --verbose --output "${outfile}" --discard_start_percent 0 --discard_end_percent 0 --normalize --normalize_fun mean --transform_type "rigid" --skip_scale_im --joint_normalize --skip_mask --scale_dicom_tags --override_dicom_naming --zoom 320

     python $preprocess_bin --path_base "${dicom_data}/${sub_dir}" --verbose --output "${outfile}" --discard_start_percent 0 --discard_end_percent 0 --normalize --normalize_fun mean --transform_type "rigid" --skip_scale_im --joint_normalize --skip_mask --scale_dicom_tags --override_dicom_naming --zoom 320 > ${logfile} 2>${errfile}
