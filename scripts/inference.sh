...
MATCH_SCALES_FSL=${MATCH_SCALES_FSL:=0}
RESHAPE_FOR_MPR_ROTATE=${RESHAPE_FOR_MPR_ROTATE:=0}
UNDO_PAD_RESAMPLE=${UNDO_PAD_RESAMPLE:=0}
USE_RESPATH=${USE_RESPATH:=0}
BRAIN_CENTERING=${BRAIN_CENTERING:=0}
RESAMPLE_ISOTROPIC=${RESAMPLE_ISOTROPIC:=1}
...
if [[ ${MATCH_SCALES_FSL} -eq "0" ]] ; then
	match_scales_fsl=" "
else
	match_scales_fsl="--match_scales_fsl "

fi

if [[ ${RESHAPE_FOR_MPR_ROTATE} -eq "0" ]] ; then
	reshape_for_mpr_rotate=" "
else
	reshape_for_mpr_rotate="--reshape_for_mpr_rotate "

fi

if [[ ${UNDO_PAD_RESAMPLE} -eq "0" ]] ; then
	undo_pad_resample=" "
else
	undo_pad_resample="--undo_pad_resample "

fi

if [[ ${USE_RESPATH} -eq "0" ]] ; then
	use_respath=" "
else
	use_respath="--use_respath "

fi

if [[ ${BRAIN_CENTERING} -eq "0" ]] ; then
	brain_centering=" "
else
	brain_centering="--brain_centering "
...
cat ${DATA_LIST_TEST} | xargs -n1 -I{} python inference.py --data_preprocess ${DATA_DIR}/{}.${FILE_EXT} --path_base ${DATA_RAW}/{} --path_out ${out_dir}/{}/{}_SubtleGad ${batch_norm_str} ${learn_residual_str} ${positive_only_str} ${split_str} ${multiprocessing_str} ${zoom_str} ${override_str} ${denoise_str} ${series_num_str} --description ${DESCRIPTION} --num_workers ${NUM_WORKERS} --max_queue_size ${QUEUE_SIZE} --verbose --slices_per_input ${SLICES_PER_INPUT} --num_channel_first ${NUM_CHANNEL_FIRST} ${inference_mpr} ${inference_mpr_avg} --num_rotations ${NUM_ROTATIONS} --resample_isotropic ${RESAMPLE_ISOTROPIC} ${resize} ${resample_size} ${brain_only} ${match_scales_fsl} ${reshape_for_mpr_rotate} ${undo_pad_resample} ${use_respath} ${brain_centering} --gpu ${GPU} --checkpoint ${CHECKPOINT_DIR}/${checkpoint_file} --log_dir ${TB_DIR} --stats_file ${LOG_DIR}/metrics/${DESCRIPTION}/{}.h5 --id ${job_id} > ${LOG_DIR}/${log_file} 2>&1
