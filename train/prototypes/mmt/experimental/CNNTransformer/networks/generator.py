...
class SubtleGeneratorUNet(nn.Module):
    def __init__(self, config, img_size=256, vis=False):
        ...
        for _ in range(self.n_contrast):
            encoder = UNet(in_channels=config.encoder['in_channels'], out_channels=config.encoder['out_channels'],
                           base_filter=config.encoder['base_filter'])
        ...
        self.latent_size = config.patch_size**2*config.encoder['out_channels']
        self.embeddings = Embeddings(in_channels=config.encoder['out_channels'], window_size=config.window_size,
        ...
        for _ in range(self.n_contrast):
            decoder = UNet(in_channels=config.decoder['in_channels'], out_channels=config.decoder['out_channels'],
                           base_filter=config.decoder['base_filter'], activation=nn.ReLU())
            self.decoders.append(copy.deepcopy(decoder))
    ...
    def encode_imgs(self, img_list, contrasts):
        ...
        for contrast, img in zip(contrasts, img_list):
            x = self.encoders[contrast](img)
            img_codes.append(x)
        return img_codes

    def decode_imgs(self, img_codes, contrasts):
        imgs = []
        for contrast, code in zip(contrasts, img_codes):
            imgs.append(self.decoders[contrast](code))
        return imgs
    ...
    def forward(self, inputs, targets, img_list=None, img_codes=None):
        # img -> encoder -> embeddings -> transformer_enc -> transformer_dec -> decoder

        #encoding + embeddings
        if not img_codes:
            img_codes = self.encode_imgs(img_list, inputs)  #[(B*n_patch, window_size**2*len(inputs), hidden)*len(inputs)]
        B = img_codes[0].shape[0]
        embeddings = self.embed_imgs(img_codes, inputs)

        # fuse latent codes
        enc_outputs, _ = self.transformer_enc(embeddings)

        contrast_tokens = [getattr(self, f'contrast_token{target}').repeat(enc_outputs.shape[0], 1, 1) for target in targets]
        contrast_token = torch.cat(contrast_tokens, 1)

        dec_outputs, _ = self.transformer_dec(contrast_token, enc_outputs)   # (B*n_patch, len(targets) * self.output_patches, hidden)

        img_outputs = []
        img_codes_output = []
        # decoding
        for i, target_contrast in enumerate(targets):
            # pdb.set_trace()
            x = dec_outputs[:, i*self.output_patches:(i+1)*self.output_patches, :]
            x = self.deembedding(x, B)  # (B, img_size, img_size, encoder.output_channel)
            img = self.decoders[target_contrast](x)
            img_codes_output.append(x)
            img_outputs.append(img)

        return img_outputs, img_codes_output
