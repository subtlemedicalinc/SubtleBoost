...
def trainer_brats(args, G, D, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i_batch, data in enumerate(tqdm(trainloader)):
            ...
            img_codes = model_G.encode_imgs(img_inputs, contrast_inputs)
            img_inputs_recon = model_G.decode_imgs(img_codes, contrast_inputs)
            img_outputs, img_outputs_c = G(contrast_inputs, contrast_outputs, img_codes=img_codes)
            #img_outputs_c_rec = model_G.encode_imgs(img_outputs, contrast_outputs)
            img_outputs_c_rec = model_G.encode_imgs(img_targets, contrast_outputs)
            ...
            loss_output_c_rec = recon_loss(img_outputs_c_rec, img_outputs_c, criterion_latent)
            ...
            opt_G.zero_grad()
            loss_g = loss_weights[0]*loss_input_rec + loss_weights[1]*loss_output_rec + loss_weights[2]*loss_output_c_rec + loss_weights[3]*loss_gan_g
            loss_g.backward()
            opt_G.step()

            opt_D.zero_grad()
            loss_d = loss_weights[3]*loss_gan_d
            loss_d.backward()
            opt_D.step()
            ...
            writer.add_scalar('train/loss_output_c_rec', loss_output_c_rec, iter_num)
