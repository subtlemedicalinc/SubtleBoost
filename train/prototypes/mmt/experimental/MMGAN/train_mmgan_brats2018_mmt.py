...
from evaluator_mmt import AverageMeter, evaluator_brats
import time
import itertools
import pickle, gc
...
train_cases = glob.glob(os.path.join(opt.data_path, "train", "BraTS*"))
val_cases = glob.glob(os.path.join(opt.data_path, "val", "BraTS*"))
...
for epoch in range(opt.epoch, opt.n_epochs, 1):
    ...
    for idx_pat, patient in enumerate(train_cases):
        ...
        for _num, batch_idx in enumerate(batch_indices):
            ...
            offset = 50
            current_slices = [os.path.join(patient, f"{i:03d}.npy") for i in range(batch_idx + offset, batch_idx + offset + opt.batch_size)]
            logger.debug("\tSplicing batch from x_real")
            x_r = []
            x_r_160 = []
            for current_slice in current_slices:
                data = torch.from_numpy(np.load(current_slice)).unsqueeze(0).cuda().float()
                #x_r_160.append(data)
                if data.shape[-1] != 256:
                    data = resize(data, (256, 256)).detach()
                x_r.append(data)
                
            x_r = torch.cat(x_r, 0)
            #x_r_160 = torch.cat(x_r_160, 0)
            ...
            #fake_x_160 = resize(fake_x, (160, 192))
...
#             metrics = evaluator_brats(generator, copy.deepcopy(scenarios), val_cases, None, vis_dir=None)
#             result_dict_test = {}
...
#                     mse_meter.update(submetric['mse'])
#                     ssim_meter.update(submetric['ssim'])
#                     psnr_meter.update(submetric['psnr'])
            
#             result_dict_test = {'mse': mse_meter.avg, 'ssim': ssim_meter.avg, 'psnr': psnr_meter.avg}
            result_dict_test = {'mse': 1, 'ssim': 1, 'psnr': 1}
    ...
    logger.info('[Testing] num_pats: {}, mse: {:.5f}, psnr: {:.5f}, ssim: {:.5f}'.format(
        opt.test_pats,
        result_dict_test['mse'],
        result_dict_test['psnr'],
        result_dict_test['ssim']
    ))

    train_hist['D_losses'].append(torch.mean(torch.FloatTensor(D_losses)))
    train_hist['G_losses'].append(torch.mean(torch.FloatTensor(G_losses)))

    train_hist['test_loss']['mse'].append(result_dict_test['mse'])
    train_hist['test_loss']['psnr'].append(result_dict_test['psnr'])
    train_hist['test_loss']['ssim'].append(result_dict_test['ssim'])
