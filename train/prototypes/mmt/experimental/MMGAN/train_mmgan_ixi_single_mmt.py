...
train_cases = glob.glob(os.path.join(opt.data_path, "train", "IXI*"))
val_cases = glob.glob(os.path.join(opt.data_path, "val", "IXI*"))
...
for epoch in range(opt.epoch, opt.n_epochs, 1):
    ...
    for idx_pat, patient in enumerate(train_cases):
        ...
        n_slices = len(glob.glob(os.path.join(patient, "*.npy"))) 
        n_slices = n_slices - n_slices%opt.batch_size
        batch_indices = list(range(0, n_slices, opt.batch_size))
        ...
        for _num, batch_idx in enumerate(batch_indices):
            ...
            logger.info(" E [{}/{}] P #{} ".format(epoch, opt.n_epochs,
                            ...
                            (_num + 1), len(batch_indices), torch.mean(torch.FloatTensor(D_real_losses)),
