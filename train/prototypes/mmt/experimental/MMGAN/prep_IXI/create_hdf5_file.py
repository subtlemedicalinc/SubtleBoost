...
import os
import glob
from prep_IXI import dataloader
import logging
import numpy as np
import h5py
import sys
...
def main():
    ...
    dataset_splits =  glob.glob(os.path.join(config['data_dir_prefix'], 'test'))[0]
    ...
    grade_type = glob.glob(os.path.join(config['data_dir_prefix'], 'train'))[0]
    ...
    grade_type = glob.glob(os.path.join(config['data_dir_prefix'], 'val'))[0]
