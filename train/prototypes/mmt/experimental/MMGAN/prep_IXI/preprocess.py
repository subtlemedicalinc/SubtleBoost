...
import h5py
from prep_IXI.configfile import config
import numpy as np
import SimpleITK as sitk
import optparse
import logging
# from modules.mischelpers import *
from prep_IXI.dataloader import standardize
import os
import pdb
...
filename = os.path.join(os.sep.join(config['hdf5_filepath_prefix'].split(os.sep)[0:-1]), 'IXI_Cropped_Normalized_Unprocessed.h5')
...
run_on_list = ['training_data_segmasks_hgg', 'training_data_hgg', 'validation_data']

#only run the mean/var normalization on these datasets
#std_list = ['training_data_hgg', 'training_data_lgg']
std_list = ['training_data_hgg']
