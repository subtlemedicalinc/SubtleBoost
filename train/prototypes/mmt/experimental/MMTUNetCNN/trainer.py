...
def trainer_brats(args, G, D, snapshot_path):
    ...
    if args.optimizer == 'sgd':
        ...
        opt_D = optim.AdamW(D.parameters(), lr=base_lr_d)
    else:
    ...
    for epoch_num in trange(args.start_epoch, max_epoch):
        ...
        for i_batch, data in enumerate(tqdm(trainloader)):
            ...
            img_outs = G(img_inputs, contrast_inputs, contrasts)
            #img_outputs = G(img_inputs, contrast_inputs, contrast_outputs)
        ...
        torch.save({'G': model_G.state_dict(), 'D': model_D.state_dict(), 'epoch': epoch_num,
                    'ssim': best_performance_ssim,
                    'mse': best_performance_mse, 'mae': best_performance_mae, 'psnr': best_performance_psnr},
                   save_mode_path)
