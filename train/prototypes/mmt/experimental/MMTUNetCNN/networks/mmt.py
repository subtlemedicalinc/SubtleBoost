...
class ResBlock(nn.Module):
    def __init__(self, dim, kernel_size=3):
        ...
        self.conv2 = nn.Conv2d(dim, dim, kernel_size, padding=1)
        self.act1 = nn.ReLU()
        self.act2 = nn.ReLU()
...
class OutputLayer(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.model = nn.Sequential(ResBlock(in_channels, kernel_size=3),
                                   ResBlock(in_channels, kernel_size=3),
...
class MMT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, out_chans=1,
        ...
        if self.final_upsample == "expand_first":
            ...
            self.output = OutputLayer(in_channels=embed_dim//patch_size**2, out_channels=self.out_chans)
