...
class MMT(nn.Module):
    ...
    def forward(self, x, inputs, outputs, return_attention=False):
        # encode separately in the encoder
        enc_out = [[] for i in range(self.num_layers)]
        for i, input_i in enumerate(inputs):
            inputs_i = [input_i]
            contrast_embeds_i = [self.contrast_embeds[j].weight[inputs_i] for j in range(self.num_layers)]
            enc_out_i = self.forward_encoder([x[i]], inputs_i, contrast_embeds=contrast_embeds_i)
            for j in range(len(enc_out_i)):
                enc_out[j].append(enc_out_i[j])
        for i in range(len(enc_out)):
            enc_out[i] = torch.cat(enc_out[i], dim=1)
        img_outputs = []
        att_maps = []
        ...
        for output in outputs:
            ...
            x, att_map = self.forward_decoder(tgt, enc_out, contrast_embeds=contrast_embeds, contrast_tokens=contrast_tokens, return_attention=return_attention)
            att_maps += att_map
            x = self.up_x4(x)
