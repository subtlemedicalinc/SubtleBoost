# mmt without CNN heads and tails
import torch
...
class MMT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, out_chans=1,
        ...
        for _ in range(self.n_contrast):
            head = LinearLayer(in_channels=patch_size**2, out_channels=embed_dim)
            self.heads.append(head)

        self.tails = nn.ModuleList()
        for _ in range(self.n_contrast):
            tail = LinearLayer(in_channels=embed_dim, out_channels=patch_size**2)
    ...
    def encode_imgs(self, img_list, contrasts):
        ...
        for contrast, img in zip(contrasts, img_list):
            x = rearrange(img, 'b c (h p1) (w p2) -> b (p1 p2 c) h w', p1=self.patch_size, p2=self.patch_size)
            x = self.heads[contrast](x)
    ...
    def forward_encoder(self, img_list, inputs, contrast_embeds=None):
        img_codes = self.encode_imgs(img_list, inputs)   # list of B, c, H, W
    ...
    def up_x4(self, x):
        ...
        x = rearrange(x, 'b n h w (p1 p2 c)-> b n (h p1) (w p2) c', n=n_contrast,
                      p1=self.patch_size, p2=self.patch_size, c=C//(self.patch_size ** 2))
        output = []
    ...
    def tail_decode(self, x, contrasts):
        output = []
        for i, contrast in enumerate(contrasts):
            x_i = x[:, i, :, :, :].permute(0, 3, 1, 2)  #B, C, h, w
            x_i = self.tails[contrast](x_i)  #B, patch_size**2, h, w
            output.append(x_i.permute(0, 2, 3, 1))  #B, h, w, patch_size**2
        return torch.stack(output, dim=1)  #B, n_contrast, h, w, patch_size**2
