...
for learning_rate in ${LEARNING_RATES[@]} ; do
	for learn_residual in ${LEARN_RESIDUALS[@]} ; do
		for batch_norm in ${BATCH_NORMS[@]} ; do
			...
			LEARNING_RATE=${learning_rate} BATCH_NORM=${batch_norm} LEARN_RESIDUAL=${learn_residual} L1_LAMBDA=.2 SSIM_LAMBDA=.8 ./scripts/train.sh
